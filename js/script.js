...
function createSceneRenderWorld() {
    ...
    scene.background = new THREE.Color(0x000000); // Ensure proper background
...
function updateSceneRenderWorld() {
    ...
    if (lives == 0) {
        ...
        // Show the endgame container and its elements
        $('#endgame').css('display', 'flex').show();
        $('#gameover').show();
        $('#try').show();
        $('#totalscore').html('Total Score: ' + score).show();
        
        // Remove any existing mint buttons first
        $('.mint-nft-button').remove();
        
        // Create single mint button if MetaMask is available
        if (typeof window.ethereum !== 'undefined') {
            const mintButton = $('<button>')
                .addClass('mint-nft-button')
                .text('Mint Score NFT');
            
            // Add minting button to the endgame-controls container
            $('#endgame-controls').append(mintButton);
            
            // Add click handler for minting
            mintButton.on('click', async function() {
                ...
                try {
                    // Initialize Web3
                    if (typeof window.ethereum === 'undefined') {
                        throw new Error('Web3 not found. Please make sure MetaMask is installed.');
                    }
                    
                    const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });
                    const account = accounts[0];
                    
                    $(this).text('Minting NFT...');
                    ...
                    // Call the mintLevelNFT function with the numeric score
                    await window.mintLevelNFT(finalScore);
                    
                    // Update button to show success
                    $(this).text('NFT Minted Successfully!')
                        .css('background-color', '#45a049');
                } catch (error) {
                    console.error('Error minting NFT:', error);
                    $(this).prop('disabled', false)
                        .text('Try Minting Again')
                        .css('background-color', '#f44336');
                }
            });
        }

        // Clean up the scene but keep camera
...
function nextGameLevel(){
    ...
    // Get current level and update display
    const currentLevel = parseInt($('#level').text().split(': ')[1]);
...
$(document).ready(function() {
    ...
    $('#level, #lives, #score, #help, #volume, #wallet-section').css('visibility', 'visible');
    ...
    // Make sure the renderer's canvas is behind UI elements
    $(renderer.domElement).css('z-index', '0');
